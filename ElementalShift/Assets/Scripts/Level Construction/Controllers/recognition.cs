...
public class MatchRecognizer : Controller {
	...
	private GridSlotGroup GetBestSquare(int startX, int startY) {
		...
		string patternMatchType = GameController.Board.gameBoard[startX, startY].GetContentType();
	...
	public void SetMatch(int startX, int startY, GridSlotGroup match) {
		string matchType = GameController.Board.gameBoard[startX, startY].GetContentType();
